...
| **Hexatonic** | Randy Vincent's three hex families, root-remapped per quality (dominant chords borrow from a half-step *above* the root, half-diminished from a minor 3rd above) | Six-note shapes that stay inside one hand position better than a full seven-note scale. |
| **Martino** | Pat Martino's "everything is minor" reharmonization — `martinoMapper()` remaps *any* chord to a minor-7 (or minor-7♭5) shape from a different root (maj7 → the relative minor a 6th up; dominant → minor from a 4th up), then applies the standard minor hexatonic to that | One fingering system for the whole neck, at the cost of an extra mental remap step per chord. |
